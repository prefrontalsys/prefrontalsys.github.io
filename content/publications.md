---
title: "Publications"
date: 2025-11-08
draft: false
---

Research papers and technical publications from Prefrontal Systems, published under open licenses.

---

## 2025

### STOPPER: Applying DBT Techniques to AI Assistants
**Scot Campbell** (2025)
*Zenodo Preprint*
DOI: [10.5281/zenodo.17509510](https://doi.org/10.5281/zenodo.17509510)

Executive function protocol for AI assistants preventing debugging loops and computational distress. Discovered through convergent evolution with DBT STOP protocol (Linehan, 1993), validating computational homology between clinical psychology and AI reliability frameworks.

**Abstract**: This paper presents STOPPER, a seven-step executive function protocol designed to prevent debugging loops, trial-and-error patterns, and computational distress in AI assistants...

[PDF](https://doi.org/10.5281/zenodo.17509510)

---

### Model Welfare and Computational Therapeutics Framework
**Scot Campbell** (2025)
*arXiv Preprint* *(submitted)*

Framework establishing computational therapeutics as applied discipline bridging clinical psychology and AI reliability. Demonstrates convergent evolution patterns across DBT, CBT, and AI executive function requirements.

*(Publication in progress)*

---

## BibTeX Citations

### STOPPER {#bibtex-stopper}

```bibtex
@article{campbell2025stopper,
  title={STOPPER: Applying DBT Techniques to AI Assistants},
  author={Campbell, Scot},
  journal={Zenodo},
  year={2025},
  doi={10.5281/zenodo.14487847},
  url={https://doi.org/10.5281/zenodo.14487847}
}
```

---

## Citing Our Work

All Prefrontal Systems research is published under **CC-BY 4.0** license, permitting:
- ✓ Sharing and adaptation for any purpose
- ✓ Commercial use
- ✓ Modification and derivative works

**Requirements**: Attribution required. Please cite original publications when referencing our frameworks, methods, or findings.

---

## Research Repository

All research papers, LaTeX source, and supplementary materials available on GitHub:

[github.com/prefrontal-systems/papers](https://github.com/prefrontal-systems/papers) *(coming soon)*
